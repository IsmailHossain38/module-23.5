        ...
        if form.is_valid():
            ...
            try:
                if current_user.balance > amount:
                    ...
                    #sender sand mail
                    mail_sub ='TransferMoney Confirmation'
                    message =render_to_string('transactions/transfer_email.html',{'user' : self.request.user,'amount':amount, })
                    ...
                    # receiver sand mail
                    mail_sub ='Received/Cradited Money Confirmation'
                    message =render_to_string('transactions/received_money.html',{'user' : self.request.user,'amount':amount, })
                    to_email = to_user.user.email
                    send_mail= EmailMultiAlternatives(mail_sub,'',to=[to_email])
                    send_mail.attach_alternative(message,'text/html')
                    send_mail.send()
                else:
                    messages.error(request,'You have no enough money')
